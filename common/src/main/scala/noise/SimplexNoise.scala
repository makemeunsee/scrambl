/*
 * Partial (2d) scala port of the following:
 * 
...
object SimplexNoise {  // Simplex noise in 2D, 3D and 4D
  private val grad3 = Seq(Grad(1,1,0),Grad(-1,1,0),Grad(1,-1,0),Grad(-1,-1,0),
                                 Grad(1,0,1),Grad(-1,0,1),Grad(1,0,-1),Grad(-1,0,-1),
                                 Grad(0,1,1),Grad(0,-1,1),Grad(0,1,-1),Grad(0,-1,-1))

  private val p: Seq[Short] = Seq(151,160,137,91,90,15,
  ...
  private val perm: Array[Short] = (0 until 512).map(i => p(i & 255)).toArray
  private val permMod12: Array[Short] = (0 until 512).map(i => (perm(i) % 12).toShort).toArray
  ...
  private def dot(g: Grad, x: Double, y: Double): Double = g.x*x + g.y*y

  // 2D simplex noise
  def noise(xin: Double, yin: Double): Double = {
    var (n0, n1, n2) = (0d, 0d, 0d) // Noise contributions from the three corners
    ...
    val (i1, j1) = if ( x0 > y0 ) (1,0) else (0 ,1) 
